...
void main() {
  ...
  testWidgets('child が描画される', (tester) async {
    await tester.pumpWidget(wrap(CamillCard(child: const Text('テスト'))));
  ...
  testWidgets('onTap が呼ばれる', (tester) async {
    ...
    await tester.pumpWidget(
      wrap(CamillCard(onTap: () => tapped = true, child: const Text('タップ'))),
    );
  ...
  testWidgets('onTap なしでもクラッシュしない', (tester) async {
    await tester.pumpWidget(wrap(CamillCard(child: const Text('no tap'))));
    await tester.tap(find.byType(CamillCard));
    await tester.pump();
  });
